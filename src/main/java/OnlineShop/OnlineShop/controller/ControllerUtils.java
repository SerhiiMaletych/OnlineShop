...
public class ControllerUtils {
    ...
    static StringBuilder getUrlBuilder(List<String> urlArray) {
        ...
        if (urlArray.get(0).contains("woman") || urlArray.get(0).contains("man")) {
            stringBuilder.append("?gender=" + urlArray.get(0));
        } else {
            stringBuilder.append("?manufacturers=" + urlArray.get(0).replaceAll("\\s+", "+"));
        }

        for (int i = 1; i < urlArray.size(); i++) {
            if (urlArray.get(i).contains("woman") || urlArray.get(i).contains("man")) {
                stringBuilder.append("&gender=" + urlArray.get(i));
            } else {
                stringBuilder.append("&manufacturers=" + urlArray.get(i).replaceAll("\\s+", "+"));
            }
        }
